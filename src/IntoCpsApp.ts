...
export default class IntoCpsApp {
  ...
  constructor(app: Electron.App, processPlatform: String) {
    ...
    let projRoot =
      Utilities.getSystemPlatform() == "windows"
        ? this.app.getPath("documents")
        : this.app.getPath("home");
    let defaultProjectFolderPath = Path.join(projRoot, "into-cps-projects");
    defaultValues[SettingKeys.INSTALL_TMP_DIR] = Path.join(
      defaultProjectFolderPath,
      "install_downloads"
    );
    defaultValues[SettingKeys.INSTALL_DIR] = Path.join(
      defaultProjectFolderPath,
      "install"
    );
    defaultValues[
      SettingKeys.DEFAULT_PROJECTS_FOLDER_PATH
    ] = defaultProjectFolderPath;
    ...
    if (this.settings.getValue(SettingKeys.DEVELOPMENT_MODE)) {
      this.settings.setValue(
        SettingKeys.UPDATE_SITE,
        this.settings.getValue(SettingKeys.DEV_UPDATE_SITE)
      );
      this.settings.setValue(
        SettingKeys.EXAMPLE_REPO,
        this.settings.getValue(SettingKeys.DEV_EXAMPLE_REPO)
      );
    }
  ...
  public loadPreviousActiveProject() {
    let activeProjectPath = this.settings.getSetting(
      SettingKeys.ACTIVE_PROJECT
    );
    if (activeProjectPath) {
      try {
        if (!fs.accessSync(activeProjectPath, fs.constants.R_OK)) {
          ...
          console.error(
            "Could not read the active project path from settings: " +
              activeProjectPath
          );
        }
      } catch (e) {
        console.warn(e);
        console.warn(
          "Unable to set active project from settings: " + activeProjectPath
        );
      }
    }
  ...
  public setActiveProject(project: IProject) {
    ...
    this.settings.setSetting(
      SettingKeys.ACTIVE_PROJECT,
      project.getProjectConfigFilePath()
    );
    this.settings.save();
  }
  ...
  public createProject(name: string, path: string) {
    let project = new Project(
      name,
      path,
      Path.normalize(path + "/.project.json")
    );
    project.save();
    this.setActiveProject(project);
    this.trmanager.changeDataBase(
      Path.dirname(path),
      this.settings.getValue(SettingKeys.INSTALL_DIR),
      this.settings.getValue(SettingKeys.INSTALL_TMP_DIR)
    );
  }
  ...
  loadProject(path: string): IProject {
    ...
    try {
      let project = SerializationHelper.toInstance(
        new Project("", "", ""),
        content.toString()
      );
      ...
      console.error("Failed to load a project with the error: " + err);
      return null;
    }
  }
