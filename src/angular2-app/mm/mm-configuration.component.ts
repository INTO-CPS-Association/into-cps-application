...
export class MmConfigurationComponent{
	...
	removeFmu(fmu: Fmu) {
		...
		this.config.fmuInstances.filter((instance) => instance.fmu === fmu).forEach((instance) => this.removeInstanceFromForm(instance));
	...
	setFmuPath(fmu: Fmu, path: string) {
		fmu
			.updatePath(path)
			.then(() => this.zone.run(() => {}))
			.catch((error) => console.error("Error in setting FMUpath: " + error));
	...
	removeInstanceFromForm(instance: Instance) {
		...
		if (this.selectedOutputInstance === instance) this.selectOutputInstance(null);

		if (this.selectedParameterInstance === instance) this.selectParameterInstance(null);
	...
	getParameters() {
		...
		return this.selectedParameterInstance.fmu.scalarVariables.filter(
			(variable) => isCausalityCompatible(variable.causality, CausalityType.Parameter) && !this.selectedParameterInstance.initialValues.has(variable)
		);
	}

	getOutputs() {
		return this.selectedOutputInstance.fmu.scalarVariables.filter((variable) => isCausalityCompatible(variable.causality, CausalityType.Output));
	}

	getInputs() {
		return this.selectedInputInstance.fmu.scalarVariables.filter(
			(variable) => isCausalityCompatible(variable.causality, CausalityType.Input) && isTypeCompatiple(variable.type, this.selectedOutput.type)
		);
	}
	...
	onConnectionChange(checked: boolean, input: ScalarVariable) {
		...
		if (checked) {
			...
			outputsTo.splice(
				outputsTo.findIndex((pair) => pair.instance === this.selectedInputInstance && pair.scalarVariable === input),
				1
			);

			if (outputsTo.length === 0) this.selectedOutputInstance.outputsTo.delete(this.selectedOutput);
		}
	}
