...
import {
  WarningMessage,
  ErrorMessage
} from "../../intocps-configurations/Messages";
...
export class MmConfigurationComponent {
  ...
  parseConfig() {
    ...
    MultiModelConfig.parse(this.path, project.getFmusPath()).then(
      config => {
        // this.zone.run(() => {
        ...
        this.form = new FormGroup({
          ...
          instances: new FormArray(
            this.config.fmus.map(
              fmu =>
                new FormArray(
                  this.getInstances(fmu).map(
                    instance =>
                      new FormControl(instance.name, [
                        Validators.required,
                        Validators.pattern("[^.]*")
                      ])
                  ),
      ...
      //this.zone.run(() => (this.parseError = error))
    );
  }
  ...
  onNavigate(): boolean {
    ...
    if (this.form.valid) {
      ...
      return confirm(
        "The changes to the configuration are invalid and can not be saved. Continue anyway?"
      );
    }
  }
  ...
  onSubmit() {
    ...
    let prevHash: string = GitConn.GitCommands.getHashOfFile(
      this.config.sourcePath
    );
    this.config.save().then(() => {
      let traceMessage = TraceMessager.submitEditMultiModelMessage(
        this.config,
        prevHash
      );
  ...
  removeFmu(fmu: Fmu) {
    ...
    if (this.selectedInstanceFmu === fmu) this.selectInstanceFmu(null);
  ...
  setFmuPath(fmu: Fmu, path: string) {
    fmu.updatePath(path).then(() => this.zone.run(() => {}));
  ...
  removeInstanceFromForm(instance: Instance) {
    ...
    if (this.selectedInputInstance === instance) this.selectInputInstance(null);
  ...
  setParameter(parameter: ScalarVariable, value: any) {
    if (parameter.type === ScalarVariableType.Real) value = parseFloat(value);
    else if (parameter.type === ScalarVariableType.Int) value = parseInt(value);
    else if (parameter.type === ScalarVariableType.Bool) value = !!value;
  ...
  getParameters() {
    if (!this.selectedParameterInstance) return [null];

    return this.selectedParameterInstance.fmu.scalarVariables.filter(
      variable =>
        isCausalityCompatible(variable.causality, CausalityType.Parameter) &&
        !this.selectedParameterInstance.initialValues.has(variable)
    );
  }

  getOutputs() {
      let xs = this.selectedOutputInstance.fmu.scalarVariables.filter(variable =>
        isCausalityCompatible(variable.causality, CausalityType.Output)
      );
    return xs;
  }

  getInputs() {
    return this.selectedInputInstance.fmu.scalarVariables.filter(
      variable =>
        isCausalityCompatible(variable.causality, CausalityType.Input) &&
        isTypeCompatiple(variable.type, this.selectedOutput.type)
    );
  }
  ...
  isInputConnected(input: ScalarVariable) {
    ...
    return (
      pairs.filter(
        pair =>
          pair.instance === this.selectedInputInstance &&
          pair.scalarVariable === input
      ).length > 0
    );
  }

  onConnectionChange(checked: boolean, input: ScalarVariable) {
    let outputsTo: InstanceScalarPair[] = this.selectedOutputInstance.outputsTo.get(
      this.selectedOutput
    );
    ...
    if (checked) {
      if (outputsTo == null) {
        outputsTo = [];
        this.selectedOutputInstance.outputsTo.set(
          this.selectedOutput,
          outputsTo
        );
      }
