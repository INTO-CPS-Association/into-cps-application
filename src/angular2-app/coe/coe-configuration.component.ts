...
import { BoundedDifferenceConstraint, CoSimulationConfig, FixedStepAlgorithm, FmuMaxStepSizeConstraint, ICoSimAlgorithm, LiveGraph, SamplingRateConstraint, VariableStepAlgorithm, VariableStepConstraint, ZeroCrossingConstraint } from "../../intocps-configurations/CoSimulationConfig";
...
export class CoeConfigurationComponent {
    ...
    CoSimulationConfig.parse(
      ...
      .then(
        config => {
            ...
            this.algorithms = this.algorithmConstructors.map(constructor =>
            ...
            this.outputPorts = this.config.multiModel.fmuInstances
              .map(instance =>
                instance.fmu.scalarVariables
                  .filter(
                    sv =>
                      sv.type === ScalarVariableType.Real &&
                      (sv.causality === CausalityType.Output ||
                        sv.causality === CausalityType.Parameter)
                  )
                  .map(sv =>
                    this.config.multiModel.getInstanceScalarPair(
                      instance.fmu.name,
                      instance.name,
                      sv.name
                    )
            ...
            this.form = new FormGroup(
              {
                startTime: new FormControl(config.startTime, [
                  Validators.required,
                  numberValidator
                ]),
                endTime: new FormControl(config.endTime, [
                  Validators.required,
                  numberValidator
                ]),
                ...
                liveGraphColumns: new FormControl(config.liveGraphColumns, [
                  Validators.required,
                  numberValidator
                ]),
                liveGraphVisibleRowCount: new FormControl(
                ...
                global_absolute_tolerance: new FormControl(
                  config.global_absolute_tolerance,
                  [Validators.required, numberValidator]
                ),
                global_relative_tolerance: new FormControl(
  ...
  onSubmit() {
    ...
    if (override) {
      this.config.saveOverride().then(() => this.change.emit(this.path));
    } else {
      this.config.save().then(() => this.change.emit(this.path));
    }
  ...
  getOutputs(scalarVariables: Array<ScalarVariable>) {
    return scalarVariables.filter(
      variable =>
        variable.causality === CausalityType.Output ||
        variable.causality === CausalityType.Local
    );
  }

  getFilterTypes(
    scalarVariables: Array<InstanceScalarPair>,
    types: ScalarVariableType[]
  ) {
    return scalarVariables.filter(
      v => types.indexOf(v.scalarVariable.type) > -1
    );
  }

  restrictToCheckedLogVariables(
    instance: Instance,
    scalarVariables: Array<ScalarVariable>
  ) {
    return scalarVariables.filter(variable =>
      this.isLogVariableChecked(instance, variable)
    );
  }
  ...
  getConstraintName(constraint: any) {
    if (
      ...
      constraint instanceof ZeroCrossingConstraint
    )
      return "Zero Crossing";
    if (
      constraint === FmuMaxStepSizeConstraint ||
      constraint instanceof FmuMaxStepSizeConstraint
    )
      return "FMU Max Step Size";

    if (
      constraint === BoundedDifferenceConstraint ||
      constraint instanceof BoundedDifferenceConstraint
    )
      return "Bounded Difference";

    if (
      constraint === SamplingRateConstraint ||
      constraint instanceof SamplingRateConstraint
    )
